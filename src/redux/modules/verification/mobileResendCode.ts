...
import {FetchResult, handleFetchError, sagaFetch} from 'utilities/helpers/fetch';
import {AlertMode, showSagaAlert} from 'utilities/helpers/alert';

const MobileResendCode = new ReduxFetchState<TMobileResendCodeRes, undefined, string | string[]>('mobileResendCode');
...
export function* watchMobileResendCode() {
  try {
    ...
    yield put(MobileResendCode.actions.loadFailure(message));
  }
}
