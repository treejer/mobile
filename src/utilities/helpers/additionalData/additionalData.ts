...
import {elementsType} from 'utilities/helpers/additionalData/constants';
...
export const addUpdateElement = ({elementProperties, typeProperties = null, formId, isModification}: any) => {
  return new Promise(resolve => {
    Realm.open(getSchema())
      .then(realm => {
        realm.write(() => {
...
const getElementData = (formData: any, realm: any, action: string) => {
  for (const i in formData) {
    for (const j in formData[i].elements) {
      ...
      if (schemaName) {
        let data: any = realm.objects(`${schemaName}`).filtered(`parentId='${formData[i].elements[j].id}'`);
