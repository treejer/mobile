...
import {appAdditionalDataForAPI, getFormattedAdditionalDetails} from 'utilities/helpers/additionalData/functions';
import {
  DATA_UPLOAD_START,
  INCOMPLETE,
  ...
  INCOMPLETE_COUNT,
} from 'utilities/helpers/inventoryConstants';
...
export const changeInventoryStatusAndLocationId = ({inventory_id, status, locationId}, dispatch) => {
  return new Promise(resolve => {
    Realm.open(getSchema())
      .then(realm => {
        realm.write(() => {
              ...
              coordinates: [
                {
                  ...
                  currentloclong: locateTree === OFF_SITE ? markedCoords[0] : currentCoords.longitude,
                  isImageUploaded: false,
                },
              ],
        ...
        realm.write(() => {
          ...
          if (inventoryFromServer.type === 'single') {
            if (inventoryFromServer.scientificSpecies) {
              let specie = realm.objectForPrimaryKey('ScientificSpecies', `${inventoryFromServer.scientificSpecies}`);
          ...
          for (let index = 0; index < numberOfIterations; index++) {
            let coordinate;
            if (inventoryFromServer.type === MULTI && inventoryFromServer.geometry.type === 'Polygon') {
          ...
          const inventoryData = {
            ...
            lastScreen: inventoryFromServer.type === SINGLE ? 'SingleTreeOverview' : 'InventoryOverview',
            species: species,
...
export const addCdnUrl = ({inventoryID, coordinateIndex, cdnImageUrl, locationId, isSampleTree}) => {
  return new Promise((resolve, reject) => {
    Realm.open(getSchema())
      .then(realm => {
        realm.write(() => {
