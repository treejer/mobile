import {accessTypes} from 'utilities/helpers/additionalData/constants';
import {appAdditionalDataForAPISchema11} from 'utilities/helpers/additionalData/functions';
import {INCOMPLETE, INCOMPLETE_SAMPLE_TREE, PENDING_DATA_UPLOAD} from 'utilities/helpers/inventoryConstants';
  ...
  if (oldRealm.schemaVersion < schemaVersion) {
    ...
    for (const index in oldInventoryObject) {
      if (
        ...
        const appVersion = oldInventoryObject[index].status === PENDING_DATA_UPLOAD ? '1.0.2' : version;
        // adds all the data from old inventory except APP accessType
        newInventoryObject[index].additionalDetails = oldInventoryObject[index].additionalDetails.filter(
        ...
        for (const sampleIndex in oldInventoryObject[index].sampleTrees) {
          ...
          // adds all the data from old inventory except APP accessType
          newInventoryObject[index].sampleTrees[sampleIndex].additionalDetails = sampleTree.additionalDetails.filter(
            (d: any) => d.accessType === accessTypes.PRIVATE || d.accessType === accessTypes.PUBLIC,
          );
          ...
          // adds appMetadata which is used to send data to API
          newInventoryObject[index].sampleTrees[sampleIndex].appMetadata = JSON.stringify(sampleAppMetadata);
        }
      }
    }
  }
