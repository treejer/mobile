...
import {basicAppAdditionalDataForAPI} from 'utilities/helpers/additionalData/functions';
  ...
  if (oldRealm.schemaVersion < schemaVersion) {
    ...
    for (const index in oldInventoryObject) {
      if (
        ...
        const appVersion = oldInventoryObject[index].status === PENDING_DATA_UPLOAD ? '1.0.4' : version;
