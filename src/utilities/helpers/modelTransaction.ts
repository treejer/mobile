...
export async function modelTransactionWithGSN(
  config: NetworkConfig,
  contractType: ContractType,
  web3: Web3,
  wallet: string,
  model: TCreateModelForm,
  method: string,
  args: any[] = [],
  useGSN = true,
) {
  ...
  if (!useGSN) {
    return modelTransactionWithoutGsn(config, contractType, web3, wallet, model, args, method);
  }
...
export async function modelTransactionWithoutGsn(
  config: NetworkConfig,
  contractType: ContractType,
  web3: Web3,
  wallet: string,
  ...
  console.log('model transaction without GSN');
  const contract = config.contracts[contractType];
  const gasPrice = await web3.eth.getGasPrice();
