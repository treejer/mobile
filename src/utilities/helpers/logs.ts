 ...
 */
const logToDB = (logLevel: TLogLevels, {referenceId, logType, message, statusCode, logStack}: ILogDataParams) => {
    ...
    Realm.open(getSchema())
      .then(realm => {
        if (type === 'all') {
          ...
          let errorLogs = allLogs.filtered('logLevel = "ERROR"').filtered('TRUEPREDICATE SORT (timestamp DESC)');
          resolve(errorLogs);
        }
      })
      .catch(err => {
